'''
Script to train MicroStateNet models on 50 subjects using LOSO (Leave-One-Subject-Out) methodology
...
def main():
    """Main LOSO training function using unified mmf functions for microstate models"""
    # Argument parsing (same as msn.py)
    parser = argparse.ArgumentParser(description='PyTorch MicroStateNet EEG Classification with LOSO and 4-fold CV')
    ...
    for test_subject_id in range(start_subject, args.n_subjects):
        ...
        # Use the unified LOSO training function for microstate models
        result = mmf.train_loso(
            test_subject_id=test_subject_id,
            args=args,
            device=device,
            data_path=data_path,
            model_type='microstate'  # Specify microstate model type
        )
        all_results.append(result)
