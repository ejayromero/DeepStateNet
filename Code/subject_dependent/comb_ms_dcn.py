...
def extract_features_from_single_model(model, data, device):
    ...
    # Handle different data formats based on the model type
    if len(x.shape) == 4:
        ...
        if x.shape[1] == 2:
            ...
            # Check if the model is an EmbeddedMicroSNet
            model_backbone = model.module if hasattr(model, 'module') else model
            is_embedded_model = isinstance(model_backbone, EmbeddedMicroSNet) or \
                              (hasattr(model_backbone, 'microstate_embedding'))
