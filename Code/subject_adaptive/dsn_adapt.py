'''
Script to perform adaptive training on DeepStateNet (DCN + MicroStateNet fusion) models
Loads pre-trained subject-independent DCN and MSN models and fine-tunes fusion classifier
...
def main():
    """Main adaptive training function for DeepStateNet fusion models"""
    # Argument parsing
    parser = argparse.ArgumentParser(description='PyTorch DeepStateNet (DCN + MicroStateNet) Adaptive Training with K-fold CV')
    ...
    # Check for pretrained independent models (both DCN and MSN required)
    print(f"\n🔍 Checking for pretrained independent models...")
    try:
        # Test loading pretrained models to verify they exist
        test_pretrained_models = mmf.load_pretrained_models(args, project_root)
        n_dcn_models = len(test_pretrained_models['dcn'])
        n_ms_models = len(test_pretrained_models['ms'])
        n_subjects_to_process = min(args.n_subjects, n_dcn_models, n_ms_models)
        
        print(f"✅ Found {n_dcn_models} DCN models and {n_ms_models} MicroStateNet models")
        
        if n_subjects_to_process < args.n_subjects:
            print(f"⚠️ Warning: Requested {args.n_subjects} subjects but only have {n_dcn_models} DCN and {n_ms_models} MicroStateNet models.")
            print(f"Processing {n_subjects_to_process} subjects instead.")
            args.n_subjects = n_subjects_to_process
        
        # Clean up test models
        del test_pretrained_models
        
    except FileNotFoundError as e:
        print(f"❌ Error loading pretrained models: {e}")
        print(f"📋 PREREQUISITES FOR DSN ADAPTIVE TRAINING:")
        print(f"  1. Run dcn_indep.py to generate independent DCN models")
        print(f"  2. Run msn_indep.py to generate independent MSN models with same settings:")
        print(f"     --model-name {args.model_name} --n-clusters {args.n_clusters}")
        if args.use_embedding:
            print(f"     --use-embedding")
        print(f"  3. Then run dsn_adapt.py to fine-tune fusion classifier")
        sys.exit(1)

    # Vocabulary scanning for embedding models (if using embeddings)
    if args.use_embedding:
    ...
    for subject_id in range(start_subject, args.n_subjects):
        ...
        # Use unified adaptive training function for fusion models
        result = mmf.train_adaptive_subject(subject_id, args, device, data_path, model_type='dsn')
        all_results.append(result)
    ...
    print(f"\n🔄 DSN Adaptive Training Summary:")
    pretrained_accs = [r['pretrained_performance'] for r in all_results]
    improvements = [r['improvement'] for r in all_results]
    adaptive_lrs = [r['adaptive_lr'] for r in all_results]
    
    print(f"Pre-trained Performance (avg DCN+MSN): {np.mean(pretrained_accs):.2f}% ± {np.std(pretrained_accs):.2f}%")
    ...
    # DSN-specific insights
    print(f"\n🔬 DSN Fusion Insights:")
    print(f"DeepStateNet combines DCN (raw EEG) + {args.model_name} (microstate) features")
    print(f"Adaptive training fine-tunes the fusion classifier on extracted features")
    print(f"Performance improvement shows benefit of subject-specific fusion weighting")
