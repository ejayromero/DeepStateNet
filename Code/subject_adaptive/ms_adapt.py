...
Clean version with embedded model support, attention models, and new repository structure
'''
...
for test_id in test_subjects:
    ...
    if input_format == 'categorical':
        # For models that use categorical input (embedded_microsnet, attention models)
        x_test_full = x_microstates  # Shape: (batch_size, sequence_length)
        
        print(f"Test data microstate shape: {x_test_full.shape}")
        print(f"Test data microstate range: {torch.min(x_test_full).item()} to {torch.max(x_test_full).item()}")
        ...
        print(f"Test data one-hot shape: {x_test_full.shape}")
    ...
    print(f"Labels shape: {y_test_full.shape}")
