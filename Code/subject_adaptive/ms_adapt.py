...
print('==================== Start of script ms_microsnet_adapt.py! ===================')
...
for test_id in test_subjects:
    ...
    x_test_full = torch.tensor(finals_ls[test_id], dtype=torch.float32)
    if x_test_full.ndim == 4:  # If shape is (n_trials, 1, n_microstates, sequence_length)
        x_test_full = x_test_full.squeeze(1)
    y_test_full = torch.tensor(all_y[test_id], dtype=torch.long)

    # Option 2: Load pre-saved indices for ultimate consistency
...
plt.title(f'Subject {type_of_subject} MicroSNet Test Accuracies for Each Subject (After Fine-tuning)')
plt.xlabel('Subject ID')
plt.ylabel('Test Accuracy (%)')
plt.xticks(range(n_subjects), [f'S{i}' for i in range(n_subjects)], rotation=45)

plt.tight_layout()
plt.savefig(os.path.join(output_path, f'{type_of_subject}_ms_DCN_test_accuracies_finetuned.png'))
plt.close()
...
fig.suptitle(f'Subject {type_of_subject} MicroSNet Training and Validation Metrics Over Epochs', fontsize=16)
...
plt.tight_layout()
plt.savefig(os.path.join(output_path, f'{type_of_subject}_ms_DCN_training_validation_metrics.png'))
plt.legend()
...
if all_finetune_accuracies:
    ...
    fig.suptitle(f'Subject {type_of_subject} MicroSNet Fine-tuning Metrics Over Epochs', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, f'{type_of_subject}_ms_DCN_finetuning_metrics.png'))
    plt.close()

print('==================== End of script ms_microsnet_adapt.py! ===================')